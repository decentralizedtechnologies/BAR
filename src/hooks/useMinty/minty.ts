 ...
 */
export async function MakeMinty(): Promise<Minty> {
  const m = new Minty();
  await m.init();
  return m;
}
...
export class Minty {
  ...
  async getNFT(tokenId, opts) {
    ...
    } = { tokenId, metadata, metadataURI, metadataGatewayURL, ownerAddress };
  ...
  private _getFileBasename(options: ContentOptions) {
